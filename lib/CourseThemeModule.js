...
import { getVariablesString as getVariablesStringFn } from './utils/getVariablesString.js'
import { processCustomStyling as processCustomStylingFn } from './utils/processCustomStyling.js'
/**
...
class CourseThemeModule extends AbstractApiModule {
  ...
  async writeCustomLess (fwBuild) {
    ...
    processCustomStylingFn(fwBuild.courseData)

    const { customStyle, [this.attributeKey]: variables, themeCustomStyle } = fwBuild.courseData.course.data
    return Promise.all([
      this.writeFile(fwBuild, '1-variables.less', fontImportString + getVariablesStringFn(variables)),
  ...
  /**
   * @deprecated Use getVariablesString() from 'adapt-authoring-coursetheme' instead
   */
  getVariablesString (data = {}, variablesStr = '') {
    return getVariablesStringFn(data, variablesStr)
  }
  ...
  /**
   * @deprecated Use processCustomStyling() from 'adapt-authoring-coursetheme' instead
   */
