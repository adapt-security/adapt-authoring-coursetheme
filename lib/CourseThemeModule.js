...
class CourseThemeModule extends AbstractApiModule {
  ...
  getMasterCourseData (fwBuild) {
    return fwBuild.courseData[0].data.course.data
  }
   ...
   * TODO: do we need to support custom theming from all languages?
   * @param {AdaptFrameworkBuild} fwBuild Reference to the current build
   */
  async writeCustomLess (fwBuild) {
    const masterCourseData = this.getMasterCourseData(fwBuild)
    const fontImportString = await this.processFileVariables(fwBuild)
    this.processCustomStyling(fwBuild)

    const { customStyle, [this.attributeKey]: variables, themeCustomStyle } = masterCourseData
    return Promise.all([
  ...
  async writeFile (fwBuild, filename, fileContents) {
    ...
    try {
      const outputDir = `${fwBuild.dir}/src/theme/${fwBuild.configData.data._theme}/less/zzzzz`
  ...
  async processFileVariables (fwBuild) {
    ...
    const masterCourseData = this.getMasterCourseData(fwBuild)
    const fontData = masterCourseData.themeVariables._font
      ...
      await Promise.all(fontData._items.map(async f => {
        ...
        for (const font in f._files) {
          ...
          try {
            ...
            const absoluteFontPath = `${fwBuild.dir}/src/theme/${fwBuild.configData.data._theme}/${relativeFontPath}`
  ...
  processCustomStyling (fwBuild) {
    const masterCourseData = this.getMasterCourseData(fwBuild)
    const customStyling = masterCourseData.themeVariables.themeCustomStyle

    if (!customStyling) return

    masterCourseData.themeCustomStyle = customStyling

    delete masterCourseData.themeVariables.themeCustomStyle
  }
}
