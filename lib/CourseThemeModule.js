...
class CourseThemeModule extends AbstractApiModule {
  ...
  async processFileVariables(fwBuild) {
    ...
    if(fontData._items !== null) {
      ...
      fwBuild.courseData.course.data.themeVariables._font._items = undefined;
    }

    //Instruction style checkbox
    if (typeof fwBuild.courseData.course.data.themeVariables._font._fontAssignments._instruction["instruction-font-style"] !== null) {
      fwBuild.courseData.course.data.themeVariables._font._fontAssignments._instruction["instruction-font-style"] = fwBuild.courseData.course.data.themeVariables._font._fontAssignments._instruction._isInstructionItalic ? 'italic' : 'normal';

      fwBuild.courseData.course.data.themeVariables._font._fontAssignments._instruction._isInstructionItalic = undefined;
    }   

    //External fonts
    if(fontData._externalFonts !== null) {
      await Promise.all(fontData._externalFonts.map(async f => {
        if (!f) return;
  
        fontImportCSS += `@import url('${f}');\n`;
      }));
  
      fwBuild.courseData.course.data.themeVariables._font._externalFonts = undefined;
    }
