...
class CourseThemeModule extends AbstractApiModule {
  ...
  async writeCustomLess (fwBuild) {
    await this.processFileVariables(fwBuild)
  ...
  async processFileVariables (fwBuild) {
    ...
    await Promise.all(Object.entries(fontData).map(async f => {
      const [data] = await assets.find({ _id: f[1] })
      const asset = assets.createFsWrapper(data)
      try {
        await fs.writeFile(`${fontDest}/${f[1]}.${asset.data.subtype}`, await asset.read())
      } catch (e) {
        this.log('error', `failed to write ${f[1]}, ${e.message}`)
      }
    }))
  }
}
